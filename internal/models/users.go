...
type UserModelInterface interface {
	...
	ShowUser() ([]User, error)
	GetUser(id int, email string) (User, error)
	UpdateUsers(id int, email string, name string) (int, error)
...
func (m *UserModel) ShowUser() ([]User, error) {

	stmt := `SELECT id, name, email , created FROM users ORDER BY id ASC LIMIT 10`
...
func (m *UserModel) GetUser(id int,email string) (User, error) {

	stmt := `SELECT id, name, hashed_password, email FROM users WHERE email=? OR id=?`

	row := m.DB.QueryRow(stmt, email, id)
	// Initialize a new zeroed Snippet struct.
	
	var s User
	err := row.Scan(&s.ID, &s.Name, &s.HashedPassword, &s.Email)
	if err != nil {
...
func (m *UserModel) UpdateUsers(id int, email string, name string) (int, error) {
	...
	//log.Println("UPDATE: name: " + name + " | email: " + email ,"| Password: " + password, id)

	stmt := `UPDATE users SET name= ? WHERE email = ?`

	_, err := m.DB.Exec(stmt, name, email)
	if err != nil {
		return 0, err
	}
